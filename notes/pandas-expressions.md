# pandas 3.0 Expression API (`pd.col`)

Research notes supporting the `as_expr()` bridge on `Indicator` — implemented in `src/mintalib/model/indicator.py` (the original proposal doc was removed as stale).
...
**In mintalib:** project pinned to Python 3.11 (`.python-version`) so pandas 3.0.2 is available.
