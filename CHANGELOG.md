...
## 0.1.5
- A new post-build hook (`scripts/mkdocs_hooks.py`) publishes every documentation `.md` into the site output, alongside its rendered HTML. `use_directory_urls` renders a page to `<stem>/index.html`, which leaves `<stem>.md` free beside it — no collisions, and it is the URL the `.md` convention already predicts. Since `docs/index.md` already links the reference pages and those link each other, the whole doc set is navigable as markdown on one origin, which is what a documentation-fetching agent can actually follow
...
- No `docs/llms.txt`. One was added and removed within this version: it restated `index.md` and the nav, and nothing auto-fetches a project-level llms.txt — tools that support the convention fetch the *origin root*, so discovery lives in `furechan.github.io/llms.txt` (hand-written, pointing at each project's `index.md`). The reference page-name rule (the `mintalib` package page becomes the section `index.md`) stays factored into `page_name()`, which the exercise introduced
