...
## 0.1.5
- `docs/llms.txt` now links the markdown sources on the docs site itself (`https://furechan.github.io/mintalib/...`) instead of `raw.githubusercontent.com`, and a new post-build hook (`scripts/mkdocs_hooks.py`) publishes every documentation `.md` into the site output so those URLs exist. `use_directory_urls` renders a page to `<stem>/index.html`, which leaves `<stem>.md` free beside it — no collisions. The reason for the switch: agents that expand an llms.txt index commonly fetch same-origin links only and skip external ones, so the GitHub-raw links were the hop that got dropped
...
- `scripts/make-api-docs.py` now also generates `docs/llms.txt` (matching mplchart): an intro, one API-reference entry per module in `MODULES` summarized from its live `__doc__` first line, and a guides entry for the hand-written `docs/index.md`. Links point at the GitHub-raw markdown in this repo, so llms.txt moves with its targets atomically. mkdocs copies non-md files verbatim, so it publishes at `https://furechan.github.io/mintalib/llms.txt`. The reference page-name rule (the `mintalib` package page becomes the section `index.md`) is factored into `page_name()`, shared by `main()` and the llms.txt writer so the two cannot drift
