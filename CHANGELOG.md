...
- `EMA`, `DEMA`, `TEMA`, `HMA`, and `ZLEMA` now accept a period of 1 and degrade to a strict identity, including missing values
- `RMA` now preserves missing inputs as `NaN` in its output while retaining its recursive state across the gap; consequently, `RMA(1)` is a strict identity, including missing values
- Breaking: `BOP` is now the canonical parameterless per-bar `(close - open) / (high - low)` measurement, matching TA-Lib and other technical-analysis libraries; apply an explicit moving average when smoothing is desired
...
- Moved the mkdocs build output and the notebook conversion cache under `build/`: `site_dir: build/site` and mkdocs-jupyter `cache_dir: build/cache/mkdocs-jupyter`, replacing the root-level `site/` and `.cache/` directories. Both are covered by the existing `build/` ignore, so `.gitignore` loses its `site/` and `.cache/` entries (`_site/` stays — the Pages workflow still builds with `--site-dir _site/`). `inv clean` is unaffected: `setup.py clean` only removes `build/temp.*`, leaving the site and cache in place
- Expanded the PyPI metadata: `keywords` gains `pandas`, `polars`, `finance` and `quantitative-finance`, and `classifiers` gains `Development Status :: 4 - Beta`, `Intended Audience :: Financial and Insurance Industry`, `Programming Language :: Cython` and `Topic :: Office/Business :: Financial :: Investment` alongside the two existing entries. Three candidates were deliberately left out: the per-version `Programming Language :: Python :: 3.11`–`3.14` enumeration, which duplicates `requires-python` (pip resolves compatibility from that, never from classifiers) and goes stale every October; a license classifier, which setuptools rejects alongside the PEP 639 `license = "MIT"` expression; and `Typing :: Typed`, which adds nothing over the `py.typed` marker already in the sdist. `Programming Language :: Cython` is the one carrying real information — it flags the compile step, the only non-obvious thing about installing this package
- Added `context7.json`, scoping the Context7 doc index to `docs/` and `examples/` with usage rules for coding agents. The existing index (parsed 2026-03-31, never refreshed) was drawn almost entirely from `scripts/` and the since-deleted `prototypes/`, `output/`, `examples/experimental/` and `WORKFLOW.md` — 2 of ~75 snippets came from `docs/`, which barely existed then. Scoping by included folders puts the codegen scripts and any future stray directory out of reach; it takes effect on the next re-parse
- Removed the search-engine verification files — `docs/BingSiteAuth.xml` and the Google Search Console step in `pages.yml` that wrote `google3e5afa36401ddb09.html` into `_site/`. Verification now happens once at the origin: `furechan.github.io` is a single property in both Google Search Console and Bing Webmaster Tools, serving both token files at the site root, and an owner of a parent URL-prefix property owns every path under it — so `https://furechan.github.io/mintalib/` inherits ownership and a project-local token verifies nothing. The per-project `mintalib` and `mplchart` properties were deleted; project sitemaps are submitted under the origin property instead
- No `docs/llms.txt`. One was added and removed within this version: it restated `index.md` and the nav, and nothing auto-fetches a project-level llms.txt — tools that support the convention fetch the *origin root*, so discovery lives in `furechan.github.io/llms.txt` (hand-written, pointing at the repos — the repo is the agent-facing surface, the site is the human one). The reference page-name rule (the `mintalib` package page becomes the section `index.md`) stays factored into `page_name()`, which the exercise introduced
- The `indicators` and `expressions` module docstrings now state their backend up front ("This module is pandas-only / polars-only ..."), replacing the input-types note buried at the bottom of the indicators docstring (docstring changes made in the generator preludes and regenerated, with `docs/reference/` refreshed via `inv docs`)
- Playground notebooks with `%%cython` magic cells now carry "cython" in the name (`filter-vs-vector`, `slope-proto`, and `curve-proto` gained the prefix), allowing the blanket disabled ty override for `playground--/**` to be replaced by a narrow `unresolved-reference` override for `*cython*.ipynb`; all other playground notebooks are fully checked
- Removed unused dev dependencies (`mypy`, `toml`, `pyarrow`, `matplotlib`, `mplfinance`, `nbmask`, `nbfixme`) — none referenced by any script, task, test, or notebook; `nbmake` kept for ad-hoc notebook testing
