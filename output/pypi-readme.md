...
- `mintalib.core`:
    core calculation routines at numpy level, implemented in cython, with names like `calc_sma`, `calc_ema`, etc ...  
- `mintalib.functions`:
    wrapper functions to apply calculations to series and dataframes, with names like `sma`, `ema`, etc ...
- `mintalib.indicators`:
    composable interface to pandas indicators with names like `SMA`, `EMA`, etc ...
