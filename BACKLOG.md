...
- ~~Fix the off-by-one in `calc_ker`~~ — done 2026-08-22: the direction numerator now spans the same `period` changes as the volatility denominator, restoring KER to 1.0 on monotonic input and the intended KAMA adaptation speed; added regression coverage for window length and null bridging.
...
- ~~Swap the `ExprFactory` overload order in `model/expression.py` so the canonical params-first form is declared first~~ — done 2026-08-22: confirmed generated factories are params-first, then moved the expression-first `Expr.pipe` compatibility overload after the canonical form.
