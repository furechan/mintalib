...
- GitHub Pages HTML — built by **mkdocs-material** (`mkdocs.yml`) via `.github/workflows/pages.yml` on every push to `main`, deployed automatically to `https://furechan.github.io/mintalib/`. Local preview: `uv run mkdocs serve` (or `uv run inv docs --serve`). The site reference section renders the committed markdown in `docs/reference/` — it only updates when the markdown is regenerated (`uv run inv docs`) and committed. (pdoc HTML deployment was replaced by mkdocs in July 2026; pdoc remains a dependency of the markdown generator only.)

Docstrings (in the Cython `.pxi` files) are **Google style** (`Args:` sections, `period (int): description` — no space before the colon). `scripts/make-api-docs.py` converts docstrings via `pdoc.docstrings.google()`. Do not re-add the Cython `embedsignature` directive (removed July 2026) — `binding=True` provides introspection and `core.pyi` provides typed stubs; `embedsignature` duplicates signatures in the docs.

Wrapper decorators (e.g. `wrap_function`, `wrap_expression`, `wrap_indicator` in `src/mintalib/model/`) must copy `__module__`, `__name__`, and `__doc__` onto their wrappers — the docs generator uses pdoc introspection, which relies on `__module__` to decide whether a name is native to a module and silently skips functions that appear imported (unless listed in an explicit `__all__`).
